...
* Upgrade from phpBB 1.4 to phpBB 1.4 startreknl.com 1.1b1 - Installation
*
* 
****************************************************************************/
...
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Strict//EN">
          <HTML>
          <HEAD>
          <TITLE>Upgrade from phpBB 1.4 to phpBB 1.4 startreknl.com 1.1b1 - Installation</TITLE>
          </HEAD>
 <BODY BGCOLOR="<?php echo $bgcolor?>" TEXT="<?php echo $textcolor?>" LINK="#11C6BD" VLINK="#11C6BD">
  <H2 align="center">Upgrade from phpBB 1.4 to phpBB 1.4 startreknl.com 1.1b1 - Installation</H2>
...
//32) Add user_avatar field in users table
$sql = "ALTER TABLE users ADD user_avatar VARCHAR (100)";
if(!$result = mysql_query($sql, $db))
{die("- Could not add user_avatar field to users table in database.");}
else {echo "- user_avatar field has been created on users table!";}

//33) Add post_icon field in posts table
$sql = "alter table posts add post_icon varchar(50)"; 
if(!$result = mysql_query($sql, $db))
{die("- Could not add post_icon field to posts table in database.");}
else {echo "- post_icon field has been created on posts table!";}

//32) Add poll_id field in topics table
$sql = "alter table topics add poll_id int(11) NOT NULL DEFAULT '0'";
if(!$result = mysql_query($sql, $db))
{die("- Could not add poll_id field to topics table in database.");}
else {echo "- poll_id field has been created on topics table!";}

//32) Add topic_icon field in topics table
$sql = "alter table topics add topic_icon varchar(50)";
if(!$result = mysql_query($sql, $db))
{die("- Could not add topic_icon field to topics table in database.");}
else {echo "- topic_icon field has been created on topics table!";}

//33) Create sessionkeys table
$sql = "CREATE TABLE sessionkeys ( 
...
if(!$result = mysql_query($sql, $db)) {die("- Could not create sessionkeys table in database.<br>");}
		else {echo "- sessionkeys table has been created!<br>";}

//34) Create poll_data table
$sql = "CREATE TABLE poll_data (
vote_id mediumint(9) NOT NULL auto_increment,
poll_id tinyint(4) DEFAULT '0' NOT NULL, 
ip varchar(15) DEFAULT '0' NOT NULL, 
time varchar(14), 
choice tinyint(4) DEFAULT '0' NOT NULL, 
PRIMARY KEY (vote_id), 
KEY ip (ip), 
KEY id (poll_id)
)";
if(!$result = mysql_query($sql, $db)) {die("- Could not create poll_data table in database.<br>");}
		else {echo "- poll_data table has been created!<br>";}

//35) Create poll_topics table
$sql = "CREATE TABLE poll_topics ( 
poll_id tinyint(4) NOT NULL auto_increment, 
title tinytext NOT NULL, 
date date DEFAULT '0000-00-00' NOT NULL, 
choices tinytext NOT NULL, 
PRIMARY KEY (poll_id)
)";
if(!$result = mysql_query($sql, $db)) {die("- Could not create poll_topics table in database.<br>");}
		else {echo "- poll_topics table has been created!<br>";}
